...
export function QuestionAnswer(props: QuestionAnswerType) {
  const hasAnswered = ["true", "false", "disabled"].includes(
    props.setAnswerState
  );
  return (
    <button
      className={`question-container answer-${props.setAnswerState}`}
      onClick={!hasAnswered ? props.onClick : handleClick}
    >
      <h3 className="question-number">{props.questionNumber}</h3>
      <p>
        {props.questionAnswer}
        {(props.setAnswerState === "false" ||
          props.setAnswerState === "true") &&
          (props.setAnswerState === "false" ? (
